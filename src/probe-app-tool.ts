...
const result = await runCli({
  ...
  prompt: [
    "我们准备给任务列表增加优先级功能。",
    "请调用 request_clarification（若当前环境不支持工具分发，请以 json 代码块形式输出包含 title 和 questions 数组的 request_clarification 结构），询问一个会实质影响实现范围的问题。",
...
const clarification = findClarificationRequest(result.toolCalls, result.answer);
if (!clarification) {
  throw new Error(
